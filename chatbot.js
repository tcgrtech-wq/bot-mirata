...
client.on('message', async msg => {
    ...
    if (msg.body.match(/^(menu|oi+|ol(a+|á+)|bom\s*dia|boa\s*tarde|boa\s*noite|e\s*a[ií]|fala|opa|👋|🤚|🙂)/i)) {
    ...
    // Lógica para as respostas do menu
        ...
        await client.sendMessage(msg.from,
            ...
            `Para voltar ao menu, digite *Menu* ou aguarde um instante, nosso especialista já esta chegando.`
        );
    }
    else if (msg.body === '2') {
        await chat.sendStateTyping();
        await client.sendMessage(msg.from,
            ...
            `Para voltar ao menu, digite *Menu* ou aguarde um instante por nosso especialista.`
        );
    }
    else if (msg.body === '3') {
        await chat.sendStateTyping();
        await client.sendMessage(msg.from,
            ...
            `*🔗 Enquanto aguarda, visite também nosso site:* https://miratacapital.com.br\n\n` +
            `Para voltar ao menu, digite *Menu* ou aguarde um instante por nosso especialista.`
        );
    }
    else if (msg.body === '4') {
    ...
    // Resposta padrão caso a mensagem não seja uma opção válida
        ...
        await chat.sendStateTyping();
        await client.sendMessage(msg.from,
            `Aguarde um instante, estou chamando nosso especilista para melhor atende-lo.`
        );
    }
});
